...
Remove the generated workspace with the same ownership checks:

```console
uv run python examples/offline-quickstart/run.py --clean
```

The command leaves unmarked or manipulated targets untouched.

Preview the generated edition with the command printed at the end of the run. The default is:

```console
uv run python -m http.server 8080 --directory .aep-quickstart/docs
```
